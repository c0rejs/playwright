...
const STATIC_DATA = await readConfig( "./staticData.json", { "resolve": import.meta.url } );
    ...
    onPageCreated () {
        return withUtils().evaluateOnNewDocument(
            ( utils, { opts, STATIC_DATA } ) => {
                ...
                const existsAlready = "runtime" in globalThis.chrome;

                // `chrome.runtime` is only exposed on secure origins
                const isNotSecure = !globalThis.location.protocol.startsWith( "https" );
                ...
                const isValidExtensionId = str => str.length === 32 && str.toLowerCase().match( /^[a-p]+$/v );
                ...
                utils.mockWithProxy( globalThis.chrome.runtime, "sendMessage", function sendMessage () {}, sendMessageHandler );
                    ...
                    "apply": function ( target, ctx, args ) {
                        ...
                        const validateConnectInfo = ci => {
                            ...
                            Object.entries( ci ).forEach( ( [ k, v ] ) => {
                                ...
                                const MismatchError = ( propName, expected, found ) => new TypeError( errorPreamble + `Error at property '${ propName }': Invalid type: expected ${ expected }, found ${ found }.` );
                ...
                utils.mockWithProxy( globalThis.chrome.runtime, "connect", function connect () {}, connectHandler );
